...
function openModal(m) {
    ...
    document.getElementById("modal-body").innerHTML = `
        ...
        <div class="modal-description-container overflow-y-auto max-h-[120px] mb-6 px-2">
            <p class="text-white/70 text-sm leading-relaxed">${USER_DESCRIPTIONS[m.id] || "Brak opisu."}</p>
        </div>
        <div class="grid grid-cols-1 gap-3 w-full">
            <a href="${RL_TRACKER_LINKS[m.id] || '#'}" target="_blank" class="flex items-center justify-center gap-2 bg-[#00b4ff]/10 hover:bg-[#00b4ff] text-[#00b4ff] hover:text-white border border-[#00b4ff]/30 py-3 rounded-xl transition-all font-bold text-sm uppercase tracking-tighter shadow-lg active:scale-95">
                Rocket League Tracker
            </a>
            <a href="${FN_TRACKER_LINKS[m.id] || '#'}" target="_blank" class="flex items-center justify-center gap-2 bg-white/5 hover:bg-white/20 text-white border border-white/10 py-3 rounded-xl transition-all font-bold text-sm uppercase tracking-tighter shadow-lg active:scale-95">
                Fortnite Tracker
            </a>
        </div>
    `;
